...
Here, $f$ is some convex function we haven't specified, and $r(x)$ is the Radon-Nikodym derivative. Intutiively, $r(x)$ gives us a way to relate probability distributions; mathematically, it's a function that satisfies

$$
\nu(E) = \int_E r\, d\mu
$$
for measures $\nu, \mu$ and measurable sets $E$. If both measures have densities (say $p$ and $q$) with respect the the Lebesgue measure, then $r(x) = p(x) / q(x)$ is called the likelihood ratio.
...
$$
\mathcal{D}_f(P\Vert Q) = \mathbb{E}_{Q}\left[\frac{|(p/q)-1|}{2}\right]
=\frac{1}{2}\int|p-q|\,dx
$$
...
More generally, the main requirement on $f$ is that $f(u)\rightarrow 0$ as $u\rightarrow 1$, which is saying that wherever the likelihood ratio is $1$ (i.e., $p(x) = q(x)$) shouldn't contribute to the divergence between the probability distributions. This give us a nice generalization of probability divergences, since we can just provide a new $f$ that behaves nicely and we get a new divergence. 
...
$$
D_f(P||Q) = \mathbb{E}_Q\left[ \sup_t \{ tr - f^*(t) \}\right]\\
= \text{sup}_T \left\{ \mathbb{E}_Q[Tr] - \mathbb{E}_Q[f^*(t)] \right\}\\
=  \text{sup}_T\left\{ \mathbb{E}_P[T] - \mathbb{E}_Q[f^*(t)] \right\}\\
$$
